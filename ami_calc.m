...
split = 3; % i am using split to denote different processing applied to data 
...
% load(fullfile(basedir,['LSD_sch454_cat.mat']),'TS_sch454_nomean') % make sure this file matches the split you want to run (see above)
E = load('TTS86.mat');

TR=420;

concTS = E.TTS86; %T-by-nparc matrix

% concTS = zscore(concTS);
...
for numClusters=[4 5 6] %if undecided, run loop. If confident about k, you can remove or comment this loop and define k above
